...
export default function FloatingLines(props) {
  const {
    linesGradient,
  ...
  useEffect(() => {
    ...
    // Initial calcs
    const getVal = (val, type, def) => (typeof val === 'number' ? val : (enabledWaves.includes(type) ? val[enabledWaves.indexOf(type)] ?? def : def));
    ...
    const handlePointerMove = event => {
      ...
      if (propsRef.current.parallax) {
        targetParallaxRef.current.set(((x - rect.width / 2) / rect.width) * propsRef.current.parallaxStrength, (-(y - rect.height / 2) / rect.height) * propsRef.current.parallaxStrength);
      }
    };
    ...
    const renderLoop = () => {
      ...
      uniforms.animationSpeed.value = currentProps.animationSpeed;
      
      if (currentProps.linesGradient?.length > 0) {
        const stops = currentProps.linesGradient.slice(0, MAX_GRADIENT_STOPS);
      ...
      currentMouseRef.current.lerp(targetMouseRef.current, currentProps.mouseDamping ?? 0.05);
      uniforms.iMouse.value.copy(currentMouseRef.current);
      currentInfluenceRef.current += (targetInfluenceRef.current - currentInfluenceRef.current) * (currentProps.mouseDamping ?? 0.05);
      uniforms.bendInfluence.value = currentInfluenceRef.current;
      
      currentParallaxRef.current.lerp(targetParallaxRef.current, currentProps.mouseDamping ?? 0.05);
